...
- **表示倍率** — 左下のパネルで拡大・縮小、倍率表示をクリックで等倍に戻し、右端のボタンで全ノードを画面に収めます。キーボードでも `+` / `-` でズーム、`0` で等倍、`f` で全体表示できます
- **接続** — ノード右側の出力ポートから、別ノード左側へドラッグして矢印を引きます。詳細パネルの「ここから接続」やノード上で `c` キーを押すと接続モードに入り、接続先をクリック・タップ・Enter で確定できます。矢印を選ぶと、詳細パネルでその接続にラベル(events、HTTPS、read など)を付けられます
